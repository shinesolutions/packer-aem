...
| aem.keystore_password_parameter | Parameter store object which contains the [Java Keystore](https://www.digitalocean.com/community/tutorials/java-keytool-essentials-working-with-java-keystores) password used in AEM Author and Publish. This is expected to exist under /aem-opencloud/ so you can ommit it from the start. For example *stack_name*/aem-keystore-password is what goes in the config but becomes /aem-opencloud/*stack_name*/aem-keystore-password on consumption. | Mandatory | |
