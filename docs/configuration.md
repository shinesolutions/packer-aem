...
| library.aem_healthcheck_source | AEM Content HealthCheck URL.  Source URL can be: `s3://...`, `http://...`, `https://...`, or `file://...` library. | Optional | https://repo.maven.apache.org/maven2/com/shinesolutions/aem-healthcheck-content/1.3.3/aem-healthcheck-content-1.3.3.zip |
...
| aem.truststore_password_parameter | Parameter store object which contains the AEM Truststore password used in AEM Author and Publish. Each / represents a subkey in Hiera. This is expected to exist under the root "/" so you must ommit it from the start. For example aem-opencloud/stack_prefix/aem-keystore-password is what goes in the config but becomes /aem-opencloud/stack_prefix/aem-keystore-password on consumption. Note: The Parameter name cannot contain dots/periods. For more information please see: https://puppet.com/docs/puppet/5.4/hiera_automatic.html#hiera-dotted-notation and https://docs.aws.amazon.com/systems-manager/latest/userguide/sysman-parameter-name-constraints.html. Mandatory if `aem.ssl_method` is set to `granite`  | Conditional | |
