...
def add_kms_key_id(module, template, kms_key_id):
    ...
    for builder in template['builders']:
        if 'launch_block_device_mappings' in builder:
            if isinstance(builder['launch_block_device_mappings'], list):
                if isinstance(builder['launch_block_device_mappings'][0], dict):
                    for ami_block_device_mapping in builder['launch_block_device_mappings']:
                        ami_block_device_mapping.update({'kms_key_id': kms_key_id})
        else:
            module.fail_json(msg="launch_block_device_mappings not found in template at builders.")
def main():
    """
